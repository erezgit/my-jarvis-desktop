...
const createComponents = (isDarkMode: boolean) => ({
  // Headings with automatic direction detection
  h1: ({ children, ...props }: ComponentProps<'h1'>) => (
    <h1
      className={`text-3xl font-bold mb-6 ${isDarkMode ? 'text-sky-400' : 'text-gray-900'}`}
      dir="auto"
  ...
  h2: ({ children, ...props }: ComponentProps<'h2'>) => (
    <h2
      className={`text-2xl font-semibold mb-4 mt-6 ${isDarkMode ? 'text-sky-300' : 'text-gray-800'}`}
      dir="auto"
      style={{ direction: 'inherit', textAlign: 'start' }}
      {...props}
    >
  ...
  h3: ({ children, ...props }: ComponentProps<'h3'>) => (
    <h3
      className={`text-xl font-medium mb-3 mt-4 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}
      dir="auto"
      style={{ direction: 'inherit', textAlign: 'start' }}
      {...props}
    >
  ...
  h4: ({ children, ...props }: ComponentProps<'h4'>) => (
    <h4
      className={`text-lg font-medium mb-2 mt-3 ${isDarkMode ? 'text-white' : 'text-gray-800'}`}
      dir="auto"
      {...props}
    >
      {children}
  ...
  // Paragraphs and text with automatic direction
  p: ({ children, ...props }: ComponentProps<'p'>) => (
    <p
      className={`mb-4 leading-relaxed ${isDarkMode ? 'text-white' : 'text-gray-700'}`}
      dir="auto"
      style={{ direction: 'inherit', textAlign: 'start' }}
      {...props}
    >
  ...
  // Lists with automatic direction
  ul: ({ children, ...props }: ComponentProps<'ul'>) => (
    <ul
      className={`list-disc list-inside mb-4 space-y-1 ${isDarkMode ? 'text-white' : 'text-gray-700'}`}
      dir="auto"
      {...props}
    >
      {children}
  ...
  ol: ({ children, ...props }: ComponentProps<'ol'>) => (
    <ol
      className={`list-decimal list-inside mb-4 space-y-1 ${isDarkMode ? 'text-white' : 'text-gray-700'}`}
      dir="auto"
      {...props}
    >
      {children}
