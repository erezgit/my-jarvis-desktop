...
const ForwardRef$8 = /* @__PURE__ */ reactExports.forwardRef(ChevronLeftIcon);
function ClockIcon({
  title,
  titleId,
  ...props
...
const ForwardRef$7 = /* @__PURE__ */ reactExports.forwardRef(ClockIcon);
function CogIcon({
  title,
  titleId,
  ...props
...
const ForwardRef$6 = /* @__PURE__ */ reactExports.forwardRef(CogIcon);
...
const ForwardRef$5 = /* @__PURE__ */ reactExports.forwardRef(CommandLineIcon);
...
const ForwardRef$4 = /* @__PURE__ */ reactExports.forwardRef(ExclamationTriangleIcon);
function MoonIcon({
  title,
  titleId,
  ...props
...
const ForwardRef$3 = /* @__PURE__ */ reactExports.forwardRef(MoonIcon);
function SunIcon({
  title,
  titleId,
  ...props
...
const ForwardRef$2 = /* @__PURE__ */ reactExports.forwardRef(SunIcon);
function XMarkIcon({
  title,
  titleId,
  ...props
...
function isSystemMessage$1(data) {
...
function isThinkingContentItem(item) {
  return typeof item === "object" && item !== null && "type" in item && item.type === "thinking" && "thinking" in item && typeof item.thinking === "string";
}
const UI_CONSTANTS = {
...
const TOOL_CONSTANTS = {
  ...
  BASH_BUILTINS: [
    "cd",
    // Change directory - internal navigation only
    "pwd",
    // Print working directory - internal state only
    "export",
    // Set environment variables - internal state only
    "unset",
    // Unset environment variables - internal state only
    "alias",
    // Set command aliases - internal state only
    "unalias",
    // Remove command aliases - internal state only
    "history",
    // Show command history - internal state only
    "jobs",
    // List active jobs - internal state only
    "bg",
    // Move jobs to background - internal process control
    "fg",
    // Move jobs to foreground - internal process control
    "exit",
    // Exit shell - internal control
    "return",
    // Return from function - internal control
    "shift",
    // Shift positional parameters - internal control
    "break",
    // Break from loop - internal control
    "continue",
    // Continue loop - internal control
    "which"
    // Which command - safe builtin that doesn't require permission
  ],
...
function extractToolInfo(toolName, input) {
  ...
  let commands = [];
  if (extractedToolName === "Bash" && input?.command && typeof input.command === "string") {
...
function extractBashCommands(commandString) {
  const commandParts = splitCompoundCommand(commandString);
  const rawCommands = commandParts.map((part) => extractSingleBashCommand(part.trim())).filter(Boolean);
  const filteredCommands = rawCommands.filter((cmd) => {
    return !TOOL_CONSTANTS.BASH_BUILTINS.includes(cmd);
  });
  const finalCommands = filteredCommands.length > 0 ? filteredCommands : rawCommands;
  return [...new Set(finalCommands)];
}
function splitCompoundCommand(commandString) {
  const separatorPattern = TOOL_CONSTANTS.COMMAND_SEPARATORS.map(
    (sep) => sep.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
  ).join("|");
...
function extractSingleBashCommand(commandPart) {
  const cmdParts = commandPart.split(/\s+/);
  if (cmdParts.length >= 2 && TOOL_CONSTANTS.MULTI_WORD_COMMANDS.includes(
    cmdParts[0]
  )) {
...
function generateToolPatterns(toolName, commands) {
  if (toolName !== "Bash") {
    return [toolName];
  }
  return commands.map(
    (command) => command !== TOOL_CONSTANTS.WILDCARD_COMMAND ? `${toolName}(${command}:*)` : toolName
  );
}
function formatToolArguments(input) {
  if (!input) return "";
...
function convertSystemMessage(claudeData, timestamp) {
  return {
    ...claudeData,
    timestamp: timestamp ?? Date.now()
  };
}
function convertResultMessage(claudeData, timestamp) {
  return {
    ...claudeData,
    timestamp: timestamp ?? Date.now()
  };
}
function createToolMessage(contentItem, timestamp) {
...
function createToolResultMessage(toolName, content, timestamp, toolUseResult) {
...
function createThinkingMessage(thinkingContent, timestamp) {
  return {
    type: "thinking",
...
function isValidTodoItem(item) {
  ...
  const obj = item;
  return typeof obj.content === "string" && typeof obj.status === "string" && ["pending", "in_progress", "completed"].includes(obj.status) && typeof obj.activeForm === "string";
}
function extractTodoDataFromInput(input) {
  try {
...
function createTodoMessageFromInput(input, timestamp) {
...
function convertTimestampedSDKMessage(message) {
...
function createVoiceMessageFromInput(input, timestamp) {
...
function convertConversationHistory(timestampedMessages) {
  const processor = new UnifiedMessageProcessor();
  return processor.processMessagesBatch(timestampedMessages);
}
function isToolUseError(content) {
  return content.includes("tool_use_error");
}
class UnifiedMessageProcessor {
  toolUseCache = /* @__PURE__ */ new Map();
  /**
   ...
   */
  processToolResult(contentItem, context, options, toolUseResult) {
    const content = typeof contentItem.content === "string" ? contentItem.content : JSON.stringify(contentItem.content);
    if (options.isStreaming && contentItem.is_error && !isToolUseError(content)) {
    ...
    if (toolName === "Bash") {
      ...
      const command = cachedInput?.command;
   ...
   */
  handleAssistantText(contentItem, context, options) {
   ...
   */
  handleToolUse(contentItem, context, options) {
    ...
    if (contentItem.name === "ExitPlanMode") {
      const planContent = contentItem.input?.plan || "";
      const planMessage = {
        type: "plan",
        plan: planContent,
   ...
   */
  processSystemMessage(message, context, options) {
   ...
   */
  processAssistantMessage(message, context, options) {
    const timestamp = options.timestamp || Date.now();
    const messages = [];
    if (options.isStreaming && context.hasReceivedInit && message.session_id && context.onSessionId) {
      context.onSessionId(message.session_id);
    }
    const localContext = options.isStreaming ? context : {
      ...context,
   ...
   */
  processResultMessage(message, context, options) {
    const timestamp = options.timestamp || Date.now();
   ...
   */
  processUserMessage(message, context, options) {
    const timestamp = options.timestamp || Date.now();
    const messages = [];
    const localContext = options.isStreaming ? context : {
      ...context,
   ...
   */
  processMessage(message, context, options = {}) {
    const timestamp = options.timestamp || ("timestamp" in message ? new Date(message.timestamp).getTime() : Date.now());
   ...
   */
  processMessagesBatch(messages, context) {
    const allMessages = [];
    const batchContext = {
      addMessage: (msg) => allMessages.push(msg),
      ...context
    };
    this.clearCache();
...
function useStreamParser() {
  const processor = reactExports.useMemo(() => new UnifiedMessageProcessor(), []);
  const adaptContext = reactExports.useCallback(
    (context) => {
      return {
  ...
  const processClaudeData = reactExports.useCallback(
    (claudeData, context) => {
      const processingContext = adaptContext(context);
      switch (claudeData.type) {
        case "system":
          if (!isSystemMessage$1(claudeData)) {
  ...
  const processStreamLine = reactExports.useCallback(
    (line, context) => {
      try {
...
function useChatState(options = {}) {
  const { initialMessages = DEFAULT_MESSAGES, initialSessionId = null } = options;
  const memoizedInitialMessages = reactExports.useMemo(
    () => initialMessages,
    [initialMessages]
  );
  ...
  const [hasShownInitMessage, setHasShownInitMessage] = reactExports.useState(false);
  const [hasReceivedInit, setHasReceivedInit] = reactExports.useState(false);
  const [currentAssistantMessage, setCurrentAssistantMessage] = reactExports.useState(null);
  ...
  const addMessage = reactExports.useCallback((msg) => {
    setMessages((prev) => [...prev, msg]);
  }, []);
  const updateLastMessage = reactExports.useCallback((content) => {
    setMessages(
      (prev) => prev.map(
        (msg, index) => index === prev.length - 1 && msg.type === "chat" ? { ...msg, content } : msg
      )
    );
  }, []);
  ...
  const generateRequestId = reactExports.useCallback(() => {
    const requestId = generateId();
    setCurrentRequestId(requestId);
    return requestId;
  }, []);
  const resetRequestState = reactExports.useCallback(() => {
    setIsLoading(false);
    setCurrentRequestId(null);
    setCurrentAssistantMessage(null);
  }, []);
  const startRequest = reactExports.useCallback(() => {
    setIsLoading(true);
    setCurrentAssistantMessage(null);
...
function usePermissions(options = {}) {
  ...
  const [allowedTools, setAllowedTools] = reactExports.useState([]);
  const [permissionRequest, setPermissionRequest] = reactExports.useState(null);
  const [planModeRequest, setPlanModeRequest] = reactExports.useState(null);
  const [isPermissionMode, setIsPermissionMode] = reactExports.useState(false);
  const showPermissionRequest = reactExports.useCallback(
    (toolName, patterns, toolUseId) => {
  ...
  const closePermissionRequest = reactExports.useCallback(() => {
    setPermissionRequest(null);
    setIsPermissionMode(false);
  }, []);
  const showPlanModeRequest = reactExports.useCallback((planContent) => {
    setPlanModeRequest({
      isOpen: true,
      planContent
    });
  ...
  const closePlanModeRequest = reactExports.useCallback(() => {
    setPlanModeRequest(null);
    setIsPermissionMode(false);
  }, []);
  const allowToolTemporary = reactExports.useCallback(
    (pattern, baseTools) => {
  ...
  const allowToolPermanent = reactExports.useCallback(
    (pattern, baseTools) => {
      const currentAllowedTools = baseTools || allowedTools;
  ...
  const resetPermissions = reactExports.useCallback(() => {
    setAllowedTools([]);
  }, []);
  const updatePermissionMode = reactExports.useCallback(
    (mode) => {
...
function usePermissionMode() {
  const [permissionMode, setPermissionModeState] = reactExports.useState("default");
  const setPermissionMode = reactExports.useCallback((mode) => {
...
const getHistoriesUrl = (projectPath) => {
  const encodedPath = encodeURIComponent(projectPath);
  return `${API_CONFIG.ENDPOINTS.HISTORIES}/${encodedPath}/histories`;
};
const getConversationUrl = (encodedProjectName, sessionId) => {
  return `${API_CONFIG.ENDPOINTS.CONVERSATIONS}/${encodedProjectName}/histories/${sessionId}`;
};
function useAbortController() {
  const performAbortRequest = reactExports.useCallback(async (requestId) => {
    await fetch(getAbortUrl(requestId), {
      method: "POST",
      headers: { "Content-Type": "application/json" }
    });
  }, []);
  const abortRequest = reactExports.useCallback(
    async (requestId, isLoading, onAbortComplete) => {
  ...
  const createAbortHandler = reactExports.useCallback(
    (requestId) => async () => {
      try {
        await performAbortRequest(requestId);
      } catch (error) {
...
function useMessageConverter() {
  const createSystemMessageCallback = reactExports.useCallback(
    (claudeData) => {
      return convertSystemMessage(claudeData);
    },
    []
  );
  const createToolMessageCallback = reactExports.useCallback(
    (contentItem) => {
      return createToolMessage(contentItem);
    },
    []
  );
  const createResultMessageCallback = reactExports.useCallback(
    (claudeData) => {
      return convertResultMessage(claudeData);
    },
    []
  );
  const createToolResultMessageCallback = reactExports.useCallback(
    (toolName, content, timestamp, toolUseResult) => {
  ...
  const createThinkingMessageCallback = reactExports.useCallback(
    (thinkingContent) => {
      return createThinkingMessage(thinkingContent);
    },
    []
  );
  const convertTimestampedSDKMessageCallback = reactExports.useCallback(
    (message) => {
      return convertTimestampedSDKMessage(message);
    },
    []
  );
  const convertConversationHistoryCallback = reactExports.useCallback(
    (timestampedMessages) => {
...
function isTimestampedSDKMessage(message) {
  return typeof message === "object" && message !== null && "type" in message && "timestamp" in message && typeof message.timestamp === "string";
}
function useHistoryLoader() {
  ...
  const { convertConversationHistory: convertConversationHistory2 } = useMessageConverter();
  const loadHistory = reactExports.useCallback(
    async (encodedProjectName, sessionId) => {
      ...
      try {
        ...
        const convertedMessages = convertConversationHistory2(timestampedMessages);
        setState((prev) => ({
          ...prev,
...
function useAutoHistoryLoader(encodedProjectName, sessionId) {
...
function SettingsButton({ onClick }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    "button",
    {
      ...
      children: /* @__PURE__ */ jsxRuntimeExports.jsx(ForwardRef$6, { className: "w-5 h-5 text-slate-600 dark:text-slate-400" })
    }
  );
}
...
function GeneralSettings() {
  const { theme, enterBehavior, toggleTheme, toggleEnterBehavior } = useSettings();
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-6", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { "aria-live": "polite", className: "sr-only", id: "settings-announcements", children: [
      theme === "light" ? "Light mode enabled" : "Dark mode enabled",
      ".",
      " ",
      enterBehavior === "send" ? "Enter key sends messages" : "Enter key creates newlines",
      "."
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-lg font-medium text-slate-800 dark:text-slate-100 mb-4", children: "General Settings" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "text-sm font-medium text-slate-700 dark:text-slate-300 mb-2 block", children: "Theme" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex items-center gap-2", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "button",
            {
              ...
              children: [
                theme === "light" ? /* @__PURE__ */ jsxRuntimeExports.jsx(ForwardRef$2, { className: "w-5 h-5 text-yellow-500" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(ForwardRef$3, { className: "w-5 h-5 text-blue-400" }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm font-medium text-slate-800 dark:text-slate-100", children: theme === "light" ? "Light Mode" : "Dark Mode" }),
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "text-sm font-medium text-slate-700 dark:text-slate-300 mb-2 block", children: "Enter Key Behavior" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex items-center gap-2", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "button",
            {
              ...
              children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(ForwardRef$5, { className: "w-5 h-5 text-slate-600 dark:text-slate-400" }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm font-medium text-slate-800 dark:text-slate-100", children: enterBehavior === "send" ? "Enter to Send" : "Enter for Newline" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-slate-500 dark:text-slate-400", children: enterBehavior === "send" ? "Enter sends message, Shift+Enter for newline" : "Enter adds newline, Shift+Enter sends message" })
                ] })
              ]
            }
          ) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-2 text-xs text-slate-500 dark:text-slate-400", children: "Controls how the Enter key behaves when typing messages in the chat input." })
        ] })
      ] })
    ] })
  ] });
...
function SettingsModal({ isOpen, onClose }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    "div",
    {
      ...
      children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xl max-w-lg w-full max-h-[90vh] overflow-hidden", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-700", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-xl font-semibold text-slate-800 dark:text-slate-100", children: "Settings" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              ...
              children: /* @__PURE__ */ jsxRuntimeExports.jsx(ForwardRef$1, { className: "w-5 h-5 text-slate-500 dark:text-slate-400" })
            }
          )
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "overflow-y-auto max-h-[calc(90vh-120px)]", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "p-6", children: /* @__PURE__ */ jsxRuntimeExports.jsx(GeneralSettings, {}) }) })
      ] })
    }
  );
}
function HistoryButton({ onClick }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    "button",
    {
      ...
      children: /* @__PURE__ */ jsxRuntimeExports.jsx(ForwardRef$7, { className: "w-5 h-5 text-slate-600 dark:text-slate-400" })
    }
  );
}
function StopIcon({
...
function renderPermissionContent(patterns) {
  if (patterns.length === 0) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-slate-600 dark:text-slate-300 mb-3", children: "Claude wants to use bash commands, but the specific commands could not be determined." });
  }
  ...
  if (isMultipleCommands) {
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-slate-600 dark:text-slate-300 mb-2", children: "Claude wants to use the following commands:" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-wrap gap-2 mb-3", children: commandNames.map((cmd, index) => /* @__PURE__ */ jsxRuntimeExports.jsx(
        "span",
        {
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-slate-600 dark:text-slate-300 mb-3", children: [
      "Claude wants to use the",
      " ",
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "font-mono bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded text-sm", children: commandName }),
      " ",
      "command."
    ] });
  }
}
function renderPermanentButtonText(patterns) {
  if (patterns.length === 0) {
...
function PermissionInputPanel({
  ...
  onSelectionChange,
  // Optional callback for demo automation
  externalSelectedOption
  // Optional external control for demo automation
}) {
  const [selectedOption, setSelectedOption] = reactExports.useState("allow");
  const isExternallyControlled = externalSelectedOption !== void 0;
  const effectiveSelectedOption = externalSelectedOption ?? selectedOption;
  const updateSelectedOption = reactExports.useCallback(
    (option) => {
      if (externalSelectedOption === void 0) {
  ...
  reactExports.useEffect(() => {
    if (externalSelectedOption !== void 0) return;
    const options = ["allow", "allowPermanent", "deny"];
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-shrink-0 px-4 py-4 bg-white/80 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl backdrop-blur-sm shadow-sm", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3 mb-4", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "p-2 bg-amber-100 dark:bg-amber-900/20 rounded-lg", children: /* @__PURE__ */ jsxRuntimeExports.jsx(ForwardRef$4, { className: "w-5 h-5 text-amber-600 dark:text-amber-400" }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-lg font-semibold text-slate-800 dark:text-slate-100", children: "Permission Required" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mb-4", children: [
      renderPermissionContent(patterns),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm text-slate-500 dark:text-slate-400", children: "Do you want to proceed? (Press ESC to deny)" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          className: getButtonClassName(
            "allow",
            `w-full p-3 rounded-lg cursor-pointer transition-all duration-200 text-left focus:outline-none ${effectiveSelectedOption === "allow" ? "bg-blue-50 dark:bg-blue-900/20 border-2 border-blue-500 dark:border-blue-400 shadow-sm" : "border-2 border-transparent"}`
          ),
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(
            "span",
            {
              className: `text-sm font-medium ${effectiveSelectedOption === "allow" ? "text-blue-700 dark:text-blue-300" : "text-slate-700 dark:text-slate-300"}`,
              children: "Yes"
            }
          )
        }
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          className: getButtonClassName(
            "allowPermanent",
            `w-full p-3 rounded-lg cursor-pointer transition-all duration-200 text-left focus:outline-none ${effectiveSelectedOption === "allowPermanent" ? "bg-green-50 dark:bg-green-900/20 border-2 border-green-500 dark:border-green-400 shadow-sm" : "border-2 border-transparent"}`
          ),
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(
            "span",
            {
              className: `text-sm font-medium ${effectiveSelectedOption === "allowPermanent" ? "text-green-700 dark:text-green-300" : "text-slate-700 dark:text-slate-300"}`,
              children: renderPermanentButtonText(patterns)
            }
          )
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          className: getButtonClassName(
            "deny",
            `w-full p-3 rounded-lg cursor-pointer transition-all duration-200 text-left focus:outline-none ${effectiveSelectedOption === "deny" ? "bg-slate-50 dark:bg-slate-800 border-2 border-slate-400 dark:border-slate-500 shadow-sm" : "border-2 border-transparent"}`
          ),
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(
            "span",
            {
              className: `text-sm font-medium ${effectiveSelectedOption === "deny" ? "text-slate-800 dark:text-slate-200" : "text-slate-700 dark:text-slate-300"}`,
              children: "No"
            }
          )
        }
...
function PlanPermissionInputPanel({
  onAcceptWithEdits,
  onAcceptDefault,
  onKeepPlanning,
  getButtonClassName = (_, defaultClassName) => defaultClassName,
  ...
  const [selectedOption, setSelectedOption] = reactExports.useState("acceptWithEdits");
  const isExternallyControlled = externalSelectedOption !== void 0;
  const effectiveSelectedOption = externalSelectedOption ?? selectedOption;
  const updateSelectedOption = reactExports.useCallback(
    (option) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-shrink-0 px-4 py-4 bg-white/80 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl backdrop-blur-sm shadow-sm", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mb-4", children: /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm text-slate-500 dark:text-slate-400", children: "Choose how to proceed (Press ESC to keep planning)" }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          className: getButtonClassName(
            "acceptWithEdits",
            `w-full p-3 rounded-lg cursor-pointer transition-all duration-200 text-left focus:outline-none ${effectiveSelectedOption === "acceptWithEdits" ? "bg-green-50 dark:bg-green-900/20 border-2 border-green-500 dark:border-green-400 shadow-sm" : "border-2 border-transparent"}`
          ),
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(
            "span",
            {
              className: `text-sm font-medium ${effectiveSelectedOption === "acceptWithEdits" ? "text-green-700 dark:text-green-300" : "text-slate-700 dark:text-slate-300"}`,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          className: getButtonClassName(
            "acceptDefault",
            `w-full p-3 rounded-lg cursor-pointer transition-all duration-200 text-left focus:outline-none ${effectiveSelectedOption === "acceptDefault" ? "bg-blue-50 dark:bg-blue-900/20 border-2 border-blue-500 dark:border-blue-400 shadow-sm" : "border-2 border-transparent"}`
          ),
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(
            "span",
            {
              className: `text-sm font-medium ${effectiveSelectedOption === "acceptDefault" ? "text-blue-700 dark:text-blue-300" : "text-slate-700 dark:text-slate-300"}`,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          className: getButtonClassName(
            "keepPlanning",
            `w-full p-3 rounded-lg cursor-pointer transition-all duration-200 text-left focus:outline-none ${effectiveSelectedOption === "keepPlanning" ? "bg-slate-50 dark:bg-slate-800 border-2 border-slate-400 dark:border-slate-500 shadow-sm" : "border-2 border-transparent"}`
          ),
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(
            "span",
            {
              className: `text-sm font-medium ${effectiveSelectedOption === "keepPlanning" ? "text-slate-800 dark:text-slate-200" : "text-slate-700 dark:text-slate-300"}`,
              children: "No, keep planning"
            }
          )
        }
      )
...
function ChatInput({
  ...
  reactExports.useEffect(() => {
    ...
    if (textarea) {
      ...
      const maxHeight = parseInt(computedStyle.maxHeight, 10) || UI_CONSTANTS.TEXTAREA_MAX_HEIGHT;
  ...
  const handleKeyDown = (e) => {
    if (e.key === KEYBOARD_SHORTCUTS.PERMISSION_MODE_TOGGLE && e.shiftKey && e.ctrlKey && !e.metaKey && // Avoid conflicts with browser shortcuts on macOS
    !isComposing) {
      e.preventDefault();
  ...
  const handleNewlineModeKeyDown = (e) => {
    if (e.shiftKey) {
      e.preventDefault();
      onSubmit();
    }
  };
  const handleSendModeKeyDown = (e) => {
    if (!e.shiftKey) {
      e.preventDefault();
      onSubmit();
    }
  ...
  const getPermissionModeIndicator = (mode) => {
    switch (mode) {
      case "default":
        return "🔧 normal mode";
      case "plan":
  ...
  const getPermissionModeName = (mode) => {
    switch (mode) {
      case "default":
        return "normal mode";
      case "plan":
  ...
  const getNextPermissionMode = (current) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-shrink-0", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("form", { onSubmit: handleSubmit, className: "relative", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "textarea",
        {
          ...
          placeholder: isLoading && currentRequestId ? "Processing..." : "Type message...",
          rows: 1,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "absolute right-2 bottom-3 flex gap-2", children: [
        isLoading && currentRequestId && /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            type: "button",
            onClick: onAbort,
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            ...
            children: isLoading ? "..." : permissionMode === "plan" ? "Plan" : "Send"
          }
        )
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        ...
        children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "ml-2 text-slate-400 dark:text-slate-500 text-[10px]", children: "- Click to cycle (Ctrl+Shift+M)" })
        ]
      }
    )
  ] });
...
function isSystemMessage(message) {
  return message.type === "system" || message.type === "result" || message.type === "error";
}
...
function TimestampComponent({
  ...
  const [displayTime, setDisplayTime] = reactExports.useState("");
  reactExports.useEffect(() => {
    const updateTime = () => {
      setDisplayTime(
        mode === "absolute" ? formatAbsoluteTime(timestamp) : formatRelativeTime(timestamp)
      );
    };
    updateTime();
    if (mode === "relative") {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className, "aria-label": `Sent at ${displayTime}`, children: displayTime });
}
function MessageContainer({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `mb-4 flex ${justifyClass}`, children: /* @__PURE__ */ jsxRuntimeExports.jsx(
    "div",
    {
...
function isValidHunk(hunk) {
  return typeof hunk === "object" && hunk !== null && "lines" in hunk && Array.isArray(hunk.lines);
}
function isValidStructuredPatch(patch) {
  return Array.isArray(patch) && patch.every(isValidHunk);
}
function isEditToolUseResult(result) {
  return typeof result === "object" && result !== null && "structuredPatch" in result && isValidStructuredPatch(result.structuredPatch);
}
function isBashToolUseResult(result) {
  return typeof result === "object" && result !== null && "stdout" in result && typeof result.stdout === "string" && "stderr" in result && typeof result.stderr === "string";
}
function createContentPreview(content, maxPreviewLines = 5) {
...
function createEditResult(structuredPatch, fallbackContent, autoExpandThreshold = 20) {
  ...
  return {
    ...
    previewContent: shouldExpand ? void 0 : allLines.slice(0, autoExpandThreshold).join("\n")
  };
}
function createBashPreview(stdout, stderr, isError, maxPreviewLines = 5) {
  const content = isError ? stderr : stdout;
  return createContentPreview(content, maxPreviewLines);
}
function createMoreLinesIndicator(totalLines, previewLines) {
...
function CollapsibleDetails({
  ...
  const contentPreview = React3.useMemo(() => {
    const computedTotalLines = details.split("\n").length;
    if (previewContent !== void 0) {
      return {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            ...
            onClick: isCollapsible ? () => setIsExpanded(!isExpanded) : void 0,
            ...
            children: [
              icon && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-4 h-4 rounded-full flex items-center justify-center text-white text-xs", children: icon }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: label }),
              badge && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "opacity-80", children: [
                "(",
                badge,
                ")"
              ] }),
              previewSummary && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "opacity-60 text-xs ml-2", children: previewSummary }),
              isCollapsible && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "ml-1 opacity-80", children: isExpanded ? "▼" : "▶" })
            ]
          }
        ),
        shouldShowPreview && /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "pre",
                {
                  ...
                  children: contentPreview.preview
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "div",
                {
                  ...
                  children: createMoreLinesIndicator(
        ...
        hasDetails && isExpanded && /* @__PURE__ */ jsxRuntimeExports.jsx(
          "pre",
          {
...
function VoiceMessageComponent({ message }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "voice-message border rounded-lg p-4 bg-blue-50", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm text-gray-600", children: "🎵 Voice Message" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-gray-800", children: message.content })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "audio",
...
function isHooksMessage(msg) {
  return msg.type === "system" && "content" in msg && typeof msg.content === "string" && !("subtype" in msg);
}
function ChatMessageComponent({ message }) {
  const isUser = message.role === "user";
  const colorScheme = isUser ? "bg-blue-600 text-white" : "bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-100";
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    MessageContainer,
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mb-2 flex items-center justify-between gap-4", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "div",
            {
              className: `text-xs font-semibold opacity-90 ${isUser ? "text-blue-100" : "text-slate-600 dark:text-slate-400"}`,
              children: isUser ? "User" : "Claude"
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            TimestampComponent,
            {
              ...
              className: `text-xs opacity-70 ${isUser ? "text-blue-200" : "text-slate-500 dark:text-slate-500"}`
            }
          )
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("pre", { className: "whitespace-pre-wrap text-sm font-mono leading-relaxed", children: message.content })
      ]
    }
  );
}
function SystemMessageComponent({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    CollapsibleDetails,
    {
      label: getLabel(),
      details,
      badge: "subtype" in message ? message.subtype : void 0,
      icon: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "bg-blue-400 dark:bg-blue-500", children: "⚙" }),
      colorScheme: {
...
function ToolMessageComponent({ message }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    MessageContainer,
    {
      alignment: "left",
      colorScheme: "bg-emerald-100 dark:bg-emerald-900 text-emerald-800 dark:text-emerald-100",
      children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-xs font-semibold mb-2 opacity-90 text-emerald-700 dark:text-emerald-300 flex items-center gap-2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-4 h-4 bg-emerald-500 dark:bg-emerald-600 rounded-full flex items-center justify-center text-white text-xs", children: "🔧" }),
        message.content
      ] })
    }
  );
}
function ToolResultMessageComponent({
  message
}) {
  const toolUseResult = message.toolUseResult;
  let previewContent;
  ...
  if (message.toolName === "Edit" && isEditToolUseResult(toolUseResult)) {
    const editResult = createEditResult(
      ...
      message.content,
      20
      // autoExpandThreshold: auto-expand if 20 lines or fewer
    );
  ...
  const shouldShowPreview = message.toolName === "Bash" || message.toolName === "Edit" || message.toolName === "Grep";
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    CollapsibleDetails,
    {
      label: message.toolName,
      details: displayContent,
      badge: message.toolName === "Edit" ? void 0 : message.summary,
      icon: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "bg-emerald-400 dark:bg-emerald-500", children: "✓" }),
      colorScheme: {
...
function PlanMessageComponent({ message }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    MessageContainer,
    {
      alignment: "left",
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mb-3 flex items-center justify-between gap-4", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-xs font-semibold opacity-90 text-blue-700 dark:text-blue-300 flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-4 h-4 bg-blue-500 dark:bg-blue-600 rounded-full flex items-center justify-center text-white text-xs", children: "📋" }),
            "Ready to code?"
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            TimestampComponent,
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mb-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm font-medium text-blue-800 dark:text-blue-200 mb-2", children: "Here is Claude's plan:" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bg-blue-100/50 dark:bg-blue-800/30 border border-blue-200 dark:border-blue-700 rounded-lg p-3", children: /* @__PURE__ */ jsxRuntimeExports.jsx("pre", { className: "text-sm text-blue-900 dark:text-blue-100 whitespace-pre-wrap font-mono leading-relaxed", children: message.plan }) })
        ] })
      ]
    }
  );
...
function ThinkingMessageComponent({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    CollapsibleDetails,
    {
      ...
      badge: "thinking",
      icon: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "bg-purple-400 dark:bg-purple-500", children: "💭" }),
      colorScheme: {
...
function TodoMessageComponent({ message }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    MessageContainer,
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mb-3 flex items-center justify-between gap-4", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-xs font-semibold opacity-90 text-amber-700 dark:text-amber-300 flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "div",
              {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "space-y-1", children: message.todos.map((todo, index) => {
          ...
          return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start gap-2", children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 min-w-0", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `text-sm ${getStatusColor(todo.status)}`, children: todo.content }),
              todo.status === "in_progress" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-amber-600 dark:text-amber-500 italic", children: todo.activeForm })
            ] })
          ] }, index);
        }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-3 text-xs text-amber-700 dark:text-amber-400", children: [
          message.todos.filter((t) => t.status === "completed").length,
          " of",
          " ",
...
function VoiceMessageComponentWrapper({ message }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(VoiceMessageComponent, { message });
}
function LoadingComponent() {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    MessageContainer,
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs font-semibold mb-2 opacity-90 text-slate-600 dark:text-slate-400", children: "Claude" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 text-sm", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "animate-pulse", children: "Thinking..." })
        ] })
      ]
    }
  );
...
function ChatMessages({ messages, isLoading }) {
  ...
  const renderMessage = (message, index) => {
    ...
    if (isSystemMessage(message)) {
      return /* @__PURE__ */ jsxRuntimeExports.jsx(SystemMessageComponent, { message }, key);
    } else if (isToolMessage(message)) {
      return /* @__PURE__ */ jsxRuntimeExports.jsx(ToolMessageComponent, { message }, key);
    } else if (isToolResultMessage(message)) {
      return /* @__PURE__ */ jsxRuntimeExports.jsx(ToolResultMessageComponent, { message }, key);
    } else if (isPlanMessage(message)) {
      return /* @__PURE__ */ jsxRuntimeExports.jsx(PlanMessageComponent, { message }, key);
    } else if (isThinkingMessage(message)) {
      return /* @__PURE__ */ jsxRuntimeExports.jsx(ThinkingMessageComponent, { message }, key);
    } else if (isTodoMessage(message)) {
      return /* @__PURE__ */ jsxRuntimeExports.jsx(TodoMessageComponent, { message }, key);
    } else if (isVoiceMessage(message)) {
      return /* @__PURE__ */ jsxRuntimeExports.jsx(VoiceMessageComponentWrapper, { message }, key);
    } else if (isChatMessage(message)) {
      return /* @__PURE__ */ jsxRuntimeExports.jsx(ChatMessageComponent, { message }, key);
    }
    return null;
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    "div",
    {
      ...
      children: messages.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx(EmptyState, {}) : /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1", "aria-hidden": "true" }),
        messages.map(renderMessage),
        isLoading && /* @__PURE__ */ jsxRuntimeExports.jsx(LoadingComponent, {}),
...
function EmptyState() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 flex items-center justify-center text-center text-slate-500 dark:text-slate-400", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-6xl mb-6 opacity-60", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { role: "img", "aria-label": "chat icon", children: "💬" }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-lg font-medium", children: "Start a conversation with Claude" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm mt-2 opacity-80", children: "Type your message below to begin" })
  ] }) });
}
/**
 * react-router v7.9.2
...
function HistoryView({ encodedName }) {
  ...
  if (loading || !encodedName) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 flex items-center justify-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-center", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-8 h-8 border-2 border-slate-300 border-t-slate-600 rounded-full animate-spin mx-auto mb-4" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-slate-600 dark:text-slate-400", children: !encodedName ? "Loading project..." : "Loading conversations..." })
    ] }) });
  }
  if (error) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 flex items-center justify-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-center max-w-md", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-16 h-16 mx-auto mb-4 bg-red-100 dark:bg-red-900/20 rounded-full flex items-center justify-center", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
        "svg",
        {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-slate-800 dark:text-slate-100 text-xl font-semibold mb-2", children: "Error Loading History" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-slate-600 dark:text-slate-400 text-sm mb-4", children: error })
    ] }) });
  }
  if (conversations.length === 0) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 flex items-center justify-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-center", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-16 h-16 mx-auto mb-4 bg-slate-200 dark:bg-slate-700 rounded-full flex items-center justify-center", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
        "svg",
        {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-slate-800 dark:text-slate-100 text-xl font-semibold mb-2", children: "No Conversations Yet" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-slate-600 dark:text-slate-400 text-sm max-w-sm", children: "Start chatting to see your conversation history here." })
    ] }) });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 overflow-hidden", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "p-6 h-full flex flex-col", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid gap-4 flex-1 overflow-y-auto", children: conversations.map((conversation) => /* @__PURE__ */ jsxRuntimeExports.jsx(
    "div",
    {
      ...
      children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 min-w-0", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("h3", { className: "text-sm font-medium text-slate-900 dark:text-slate-100 truncate", children: [
            "Session: ",
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm text-slate-600 dark:text-slate-300 mt-2 line-clamp-2", children: conversation.lastMessagePreview })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "ml-4 flex-shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
          "svg",
          {
...
function ChatPage() {
  ...
  const [currentView, setCurrentView] = reactExports.useState(null);
  ...
  const getEncodedName = reactExports.useCallback(() => {
    if (!projects.length) {
      return null;
    }
  ...
  const {
    ...
    initialSessionId: loadedSessionId || void 0
  });
  const {
    allowedTools,
    permissionRequest,
  ...
  const handlePermissionError = reactExports.useCallback(
    (toolName, patterns, toolUseId) => {
  ...
  const sendMessage = reactExports.useCallback(
    async (messageContent, tools, hideUserMessage = false, overridePermissionMode) => {
  ...
  const handlePermissionAllow = reactExports.useCallback(() => {
  ...
  const handlePermissionAllowPermanent = reactExports.useCallback(() => {
    if (!permissionRequest) return;
    let updatedAllowedTools = allowedTools;
    permissionRequest.patterns.forEach((pattern) => {
  ...
  const handlePermissionDeny = reactExports.useCallback(() => {
    closePermissionRequest();
  }, [closePermissionRequest]);
  const handlePlanAcceptWithEdits = reactExports.useCallback(() => {
  ...
  const handlePlanAcceptDefault = reactExports.useCallback(() => {
  ...
  const handlePlanKeepPlanning = reactExports.useCallback(() => {
  ...
  const handleSettingsClose = reactExports.useCallback(() => {
  ...
  const handleBackToChat = reactExports.useCallback(() => {
    setCurrentView(null);
    setSessionId(null);
  }, []);
  const handleBackToHistory = reactExports.useCallback(() => {
    setCurrentView("history");
  }, []);
  const handleBackToProjects = reactExports.useCallback(() => {
    console.log("Back to projects - not applicable in Electron app");
  }, []);
  const handleBackToProjectChat = reactExports.useCallback(() => {
    setCurrentView(null);
  }, []);
  reactExports.useEffect(() => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-300", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-6xl mx-auto p-3 sm:p-6 h-screen flex flex-col", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between mb-4 sm:mb-8 flex-shrink-0", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-4", children: [
        isHistoryView && /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            ...
            children: /* @__PURE__ */ jsxRuntimeExports.jsx(ForwardRef$8, { className: "w-5 h-5 text-slate-600 dark:text-slate-400" })
          }
        ),
        isLoadedConversation && /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("nav", { "aria-label": "Breadcrumb", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
                ...
                children: "Claude Code Web UI"
              }
            ),
            (isHistoryView || sessionId) && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "span",
                {
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "h1",
                {
                  ...
                  "aria-current": "page",
                  children: isHistoryView ? "Conversation History" : "Conversation"
                }
              )
            ] })
          ] }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center text-sm font-mono mt-1", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
            ...
            sessionId && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "ml-2 text-xs text-slate-600 dark:text-slate-400", children: [
              "Session: ",
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3", children: [
        !isHistoryView && /* @__PURE__ */ jsxRuntimeExports.jsx(HistoryButton, { onClick: handleHistoryClick }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(SettingsButton, { onClick: handleSettingsClick })
      ] })
    ] }),
    isHistoryView ? /* @__PURE__ */ jsxRuntimeExports.jsx(
      HistoryView,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 flex items-center justify-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-center", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-8 h-8 border-2 border-slate-300 border-t-slate-600 rounded-full animate-spin mx-auto mb-4" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-slate-600 dark:text-slate-400", children: "Loading conversation history..." })
      ] }) })
    ) : historyError ? (
      /* Error loading conversation history */
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 flex items-center justify-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-center max-w-md", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-16 h-16 mx-auto mb-4 bg-red-100 dark:bg-red-900/20 rounded-full flex items-center justify-center", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
          "svg",
          {
            className: "w-8 h-8 text-red-500",
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-slate-800 dark:text-slate-100 text-xl font-semibold mb-2", children: "Error Loading Conversation" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-slate-600 dark:text-slate-400 text-sm mb-4", children: historyError }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            onClick: () => {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(ChatMessages, { messages, isLoading }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        ChatInput,
        {
          input,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(SettingsModal, { isOpen: isSettingsOpen, onClose: handleSettingsClose })
  ] }) });
}
...
function getSettings() {
  const unifiedSettings = getStorageItem(
...
function migrateLegacySettings() {
  ...
  const legacyTheme = getStorageItem(
    STORAGE_KEYS.THEME,
    systemDefaultTheme
  );
  const legacyEnterBehavior = getStorageItem(
...
function SettingsProvider({ children }) {
  ...
  const toggleEnterBehavior = reactExports.useCallback(() => {
    updateSettings({
...
function App() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(SettingsProvider, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(ChatPage, {}) });
}
